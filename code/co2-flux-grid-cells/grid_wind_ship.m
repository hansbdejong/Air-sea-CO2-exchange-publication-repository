% bins wind into lat and lon grids
...
for month = 1:5
    ...
    for j = 1:15
        ...
        for i = 1:21
            ...
            % extract grid data
            wind_section = wind(lat <=  Y(j) & lat >= Y(j+1) & lon >=  X(i) & lon <= X(i+1));
            ...
            %wind from specific grid and specific month
            wind_month = wind_section(julian_section >= start(month) & julian_section < ending(month)); 
            
            %years where wind data collected
            ...
            % average wind for each unique day 
            for k = 1:length(unique_timestamp)  
               
                % the wind values for that unique day
                wind_temp = wind_month(timestamp_month == unique_timestamp(k));
                
                % the year of that unique day
                year_temp = year_month(timestamp_month == unique_timestamp(k));
                
                % the mean wind for that day
                wind_mean_temp_1(k) = mean(wind_temp);
            ...
            % finds the average wind for each unique year - averages the
            % daily averages for that year
            for k = 1:length(unique_year) 
                
                % wind daily averages for that year
                wind_temp = wind_mean_temp_1(year_temp_mean_1 == unique_year(k));
                
                % averages the daily averages for that year
                wind_mean_temp_2(k) = mean(wind_temp);
            ...
            grid(j,i) = mean(wind_mean_temp_2);        
        end
    end
    
    % outputs file for that month
    grid = grid.*mask;
    filename = strcat(names(month), '_wind_W14_Shelf.xlsx');
