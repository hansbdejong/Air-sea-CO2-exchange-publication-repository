% bins FCO2 into lat and lon grids
...
for month = 1:5
    ...
    for j = 1:15
        ...
        for i = 1:21
            ...
            FCO2_month = FCO2_section(julian_section >= start(month) & julian_section < ending(month)); 
            ...
            % finds the average FCO2 for each unique year - averages the
            % daily averages for that year
            for k = 1:length(unique_year) 
                
                % FCO2 daily averages for that year
                FCO2_temp = FCO2_mean_temp_1(year_temp_mean_1 == unique_year(k));
                
                % averages the daily averages for that year
                FCO2_mean_temp_2(k) = mean(FCO2_temp);
            ...
            grid(j,i) = mean(FCO2_mean_temp_2);        
        end
    end
    
    % outputs file for that month
    grid = grid.*mask;
    filename = strcat(names(month), '_FCO2_W14_Shelf.xlsx');
